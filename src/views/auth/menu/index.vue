<template>
  <div class="app-container">
    ...
    <!--表单渲染-->
    <el-dialog append-to-body :close-on-click-modal="false" @close="handelCancel" :visible.sync="dialog" :title="dialogTitle" width="580px">
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item v-show="form.type.toString() !== '2'" label="菜单可见" prop="hidden">
          <el-radio-group v-model="form.hidden" size="mini" style="width: 178px">
        ...
        <el-form-item v-show="form.type.toString() === '1'" label="组件名称" prop="component_name">
          <el-input v-model="form.component_name" style="width: 178px;" placeholder="匹配组件内Name字段" />
        </el-form-item>
    ...
    <el-table ref="table"  v-loading="loading" :data="tableData" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" row-key="id">
      ...
      <el-table-column prop="hidden" label="可见" width="75px">
        <template slot-scope="scope">
          <el-tag :type="scope.row.hidden?'info':'success'">{{scope.row.hidden?'否':'是'}}</el-tag>
      ...
      <el-table-column label="操作" width="130px" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button :disabled="$permissionButton('/menus/edit')" size="mini" type="primary" icon="el-icon-edit" @click="handelEdit(scope.row)" />
          <el-button :disabled="$permissionButton('/menus/del')" size="mini" type="danger" icon="el-icon-delete" @click="handelDel(scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
const defaultForm = {id: '',name: '',sort: 999,path: '',component: '',component_name: '',pid: 0,icon: '',hidden: 0,type: 0,permission:''}
export default {
  name: 'Menu',
  inject:['reload'],
  ...
  methods: {
    ...
    handelAdd() {
      ...
      getMenusTree().then(res => {
        let _res;
        _res = this.resFun(res); //删除 children:null
        this.menus = []
    ...
    resFun(x){       //去除children: null
      let len = x.length;
      for(let i=0;i<len;i++){
        !x[i].children && (delete x[i].children);
